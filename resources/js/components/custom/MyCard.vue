...
const {
  ...
  hasBottomRight,
  cardBackgroundClass,
} = getLinkArrows(props.card);

const goToCard = () => {
  router.visit(`/cards/${props.card.id}`);
};
</script>

<template>
  <Card
    class="flex cursor-pointer flex-col transition-shadow hover:shadow-lg"
    @click="goToCard"
  >
    <CardHeader class="flex flex-0 shrink-0 flex-col">
      <CardTitle class="w-full overflow-hidden">
        <HoverCard>
          <HoverCardTrigger asChild>
            <div class="w-full overflow-hidden text-nowrap text-ellipsis">
              {{ card.name }}
            </div>
          </HoverCardTrigger>
      ...
      <CardDescription class="flex w-full flex-0 items-center justify-center">
        ...
        <span
          v-else-if="card.linkval && card.linkval > 0"
          class="grow-2 text-start"
        >
        ...
        <template v-if="card.archetype">
          ...
          <span class="mx-2 overflow-hidden text-nowrap text-ellipsis">{{
            card.archetype
          }}</span>
          <Separator orientation="vertical" class="h-full" style="width: 2px" />
        </template>
        ...
        <span v-if="card.attribute" class="shrink-0 grow-2 text-end">{{
          card.attribute
        }}</span>
      </CardDescription>
    </CardHeader>
    ...
    <CardContent
      class="relative mx-6 flex !aspect-square h-[75%] flex-1 items-center justify-center px-0"
    >
      <template v-if="card.linkmarkers">
        <Square
          v-if="hasTopLeft"
          class="absolute top-[-2.5%] left-[-2.5%] h-[20%] w-[20%] text-red-500"
          stroke-width="3"
        />
        <Square
          v-if="hasTop"
          class="absolute top-[-2.5%] left-1/2 h-[20%] w-[20%] -translate-x-1/2 text-red-500"
          stroke-width="3"
        />
        <Square
          v-if="hasTopRight"
          class="absolute top-[-2.5%] right-[-2.5%] h-[20%] w-[20%] text-red-500"
          stroke-width="3"
        />

        <Square
          v-if="hasLeft"
          class="absolute top-1/2 left-[-2.5%] h-[20%] w-[20%] -translate-y-1/2 text-red-500"
          stroke-width="3"
        />
        <Square
          v-if="hasRight"
          class="absolute top-1/2 right-[-2.5%] h-[20%] w-[20%] -translate-y-1/2 text-red-500"
          stroke-width="3"
        />

        <Square
          v-if="hasBottomLeft"
          class="absolute bottom-[-2.5%] left-[-2.5%] h-[20%] w-[20%] text-red-500"
          stroke-width="3"
        />
        <Square
          v-if="hasBottom"
          class="absolute bottom-[-2.5%] left-1/2 h-[20%] w-[20%] -translate-x-1/2 text-red-500"
          stroke-width="3"
        />
        <Square
          v-if="hasBottomRight"
          class="absolute right-[-2.5%] bottom-[-2.5%] h-[20%] w-[20%] text-red-500"
          stroke-width="3"
        />
      </template>
